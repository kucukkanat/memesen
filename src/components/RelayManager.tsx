  ...
  return (
    <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 388, zIndex: 26, boxShadow: '0 12px 38px rgba(0,0,0,.5)' }}>
      <div style={{ ...TITLE_BAR }}>
        ...
        <span style={TITLE_TEXT}>Connection — Relays</span>
      ...
      <div style={{ ...SIDE_BORDERS, background: '#fff', borderBottom: '1px solid #06387c' }}>
        <div style={{ padding: '8px 10px', color: '#33476a', fontSize: 11, background: 'linear-gradient(180deg,#eaf3ff,#d7e6fa)', borderBottom: '1px solid #b9c8de' }}>
          Messenger reads and writes to every <b>enabled</b> relay. The five most popular public relays are configured by default.
        </div>
        ...
        <div className="msn-scroll" style={{ maxHeight: 220, overflowY: 'auto' }}>
          {p.relays.map((r) => (
            <div key={r.url} style={row}>
              ...
              <span onClick={() => p.onRemove(r.url)} title="Remove relay" style={{ color: '#b04030', cursor: 'pointer', fontSize: 12, padding: '0 4px' }}>✕</span>
            </div>
          ))}
          {p.relays.length === 0 && <div style={{ padding: 14, textAlign: 'center', color: '#8a93a0', fontSize: 11 }}>No relays. Add one below to connect.</div>}
        </div>
